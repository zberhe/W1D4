...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            List<Flight> flights = session.createQuery("select f from Flight f join f.origin o join f.airplane a where "
                    + "a.capacity>500 and  o.country='USA'").list();
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("select f from Flight f where f.airplane.model='747' and f.airline.name<> 'Star Alliance'").list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("select f from Flight f where f.departureDate = '2009-08-07' and f.departureTime"
                    + "<'12:00:00'").list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
